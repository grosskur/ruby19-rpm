    ...
    def default_dirs
      @default_dirs ||= Hash[default_locations.collect do |destination, path|
        [destination, {
          ...
          :gem_dir => File.join(path, ConfigMap[:datadir].split(File::SEPARATOR).last,
                                'gems' + ConfigMap[:MAJOR] + '.' + ConfigMap[:MINOR]),
          :ext_dir => File.join(path, ConfigMap[:libdir].split(File::SEPARATOR).last,
                                'gems' + ConfigMap[:MAJOR] + '.' + ConfigMap[:MINOR])
        }]
      end]
    end
